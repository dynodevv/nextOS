...
static void install_write_image(void)
{
    ...
    /* Write up to 32 sectors per frame (~16 KiB) — fast enough to finish
     * within a few seconds, slow enough to see the progress bar move.   */
    uint32_t batch = 32;
    ...
    if (install_sectors_written >= total) {
        ...
        install_progress = 101;   /* triggers transition to step 3 */
    }
}
